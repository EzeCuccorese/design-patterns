...
*   **Gestión de Memoria (Stack vs Heap):** Comprensión del funcionamiento interno de la memoria RAM de un proceso. La **Pila (Stack)** es rápida, secuencial (LIFO) y guarda variables primitivas locales y referencias/punteros. El **Montón (Heap)** es de gran tamaño, dinámico y guarda los objetos físicos instanciados (con `new`), delegando la recolección de basura a un recolector automático (**Garbage Collector**) para prevenir fugas de memoria y errores críticos como `OutOfMemoryError` u `Overflows` en la pila.
